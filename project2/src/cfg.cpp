...
namespace whilec {
    ...
    static CFGNode* nodeFor(const Node* n, CFG& cfg) {
        ...
        if (auto a = dynamic_cast<const Assign*>(n)) lbl = a->label;
        else if (auto s = dynamic_cast<const Skip*>(n)) lbl = s->label;
        else if (auto i = dynamic_cast<const If*>(n)) lbl = i->label;
        else if (auto w = dynamic_cast<const While*>(n)) lbl = w->label;
        else
